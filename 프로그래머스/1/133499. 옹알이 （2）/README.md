...
### 문제 회고
- 정규식 사용
- 연속발음이 포함된 경우 숫자 더하지 않음
- 기가막히게 했다고 생각했는데 왜 안되나 고민했는데, **test()는** **g** **플래그와** **함께** **쓰면** **내부** **상태를** **기억**(캐싱)한다는 것을 알게되었다
  - test()는 내부적으로 lastIndex라는 포인터를 가짐
  - reduce() 같은 반복문 안에서 test()를 여러 번 쓰면 의도하지 않은 결과
  - g 는 여기서 의미가 없는데, 수정수정하다가 남은 것으로, 삭제하여 해결
